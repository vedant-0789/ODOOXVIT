...
export default function App() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (foundUser) {
      if (foundUser.password === password) {
        ...
        // Set currency based on admin's country if user is admin
        if (foundUser.role === 'Admin' && foundUser.countryCode) {
          const adminCountry = COUNTRIES.find(c => c.code === foundUser.countryCode);
          if (adminCountry) setCurrency(adminCountry);
        }
      } else {
  ...
  const handleSignup = (e: React.FormEvent) => {
    ...
    // Check if an admin already exists for this country
    const existingAdmin = users.find(u => u.role === 'Admin' && u.countryCode === countryCode);
    
    if (existingAdmin) {
      setAuthError(`An administrator already exists for ${currency.name}. Only one admin per country is allowed.`);
      return;
    }
  ...
  const handleAddUser = (e: React.FormEvent) => {
    ...
    const newUser: UserProfile = {
      ...
      avatar: `https://picsum.photos/seed/${email}/100/100`,
      countryCode: user?.countryCode // New users inherit admin's country
    };
